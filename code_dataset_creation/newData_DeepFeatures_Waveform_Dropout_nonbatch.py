...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    enhanced = featureloss_train(input1_wav,clean1_wav,loss_weights,loss_layers=SE_LOSS_LAYERS,n_layers=LOSS_LAYERS, norm_type=LOSS_NORM, base_channels=LOSS_BASE_CHANNELS,blk_channels=LOSS_BLK_CHANNELS) 
...
with tf.Session() as sess:
    ...
    opt_task = tf.train.AdamOptimizer(learning_rate=1e-3).minimize(loss_1,var_list=[var for var in tf.trainable_variables()])
    ...
    for epoch in range(epoches):
        ...
        print("Epoch {} Train Loss {}".format(epoch,loss_epoch))
        if epoch%20==0:
