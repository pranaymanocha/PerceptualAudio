#Testing model where the weight vectors 'w' are trained and then after normalising, they are multiplied
import numpy as np
#!pip install tqdm
#import tqdm
...
def argument_parser():
    ...
    parser.add_argument('--kernel_size', help='kernel size for convolutions', default=3, type=int)
    return parser
...
def waveform2spec_2(input,n_layers,kernel,reuse):
    ...
    for id in range(25,25+n_layers):
        if id == 25:
            net = slim.conv2d(input, 1, [1, kernel], activation_fn=lrelu, stride=[1, 2],
                              scope='loss_conv_%d' % id, padding='VALID', reuse=reuse)
            layers.append(net)
...
def featureloss_train(target, current, loss_weights, loss_layers, n_layers=14, norm_type="SBN", base_channels=32, blk_channels=5):

    feat_current = lossnet(current, reuse=False, n_layers=n_layers, norm_type=norm_type,
                         base_channels=base_channels, blk_channels=blk_channels,training=True,ksz=args.kernel_size)
...
def load_full_data(dataset,sets,id_value):
    ...
    inputData_wav=dataset[sets]['inwav'][id_value]
    outputData_wav=dataset[sets]['outwav'][id_value]
    ...
    for i in range(len(id_value[0])):
        ...
        #inputData_spec=dataset[sets]['inspec'][id]
        #outputData_spec=dataset[sets]['outspec'][id]
        ...
        if i==0:
            ...
            #spec_in=np.concatenate((spec_in,inputData_spec),axis=0)
            #spec_out=np.concatenate((spec_out,outputData_spec),axis=0)
...
def loadall_audio_train(dataset):
    ...
    dataset['train']['inwav']  = [None]*len(dataset['train']['inname'])
    dataset['train']['outwav'] = [None]*len(dataset['train']['outname'])
    ...
    for id in tqdm(range(len(dataset['train']['inname']))):
        ...
        if dataset['train']['inspec'][id] is None:
            ...
            try:
                ...
                dataset['train']['inwav'][id]  = inputData_fr
                #dataset['train']['outaudio'][id] = outputData_wav
                dataset['train']['outwav'][id] = outputData_fr
                
            except:
                dataset['train']['inspec'][id]  = dataset['train']['inspec'][id-1]
                #dataset['train']['outaudio'][id] = outputData_wav
                dataset['train']['outspec'][id] = dataset['train']['outspec'][id-1]
                dataset['train']['inwav'][id]  = dataset['train']['inwav'][id-1]
                #dataset['train']['outaudio'][id] = outputData_wav
                dataset['train']['outwav'][id] = dataset['train']['outwav'][id-1]
...
def loadall_audio_test(dataset):
    ...
    dataset['test']['inwav']  = [None]*len(dataset['test']['inname'])
    dataset['test']['outwav'] = [None]*len(dataset['test']['outname'])
    ...
    for id in tqdm(range(len(dataset['test']['inname']))):
        ...
        if dataset['test']['inspec'][id] is None:
            ...
            try:
                ...
                dataset['test']['inwav'][id]  = inputData_fr
                #dataset['train']['outaudio'][id] = outputData_wav
                dataset['test']['outwav'][id] = outputData_fr
            except:
                dataset['test']['inspec'][id]  = dataset['test']['inspec'][id-1]
                #dataset['train']['outaudio'][id] = outputData_wav
                dataset['test']['outspec'][id] = dataset['test']['outspec'][id-1]
                
                dataset['test']['inwav'][id]  = dataset['test']['inwav'][id-1]
                #dataset['train']['outaudio'][id] = outputData_wav
                dataset['test']['outwav'][id] = dataset['test']['outwav'][id-1]
...
#dataset=split_data_equally(dataset)
#dataset1=split_test(dataset)
#dataset=combine_lists_trainLinearReverb(dataset)
dataset_train=loadall_audio_train(dataset)
dataset_test=loadall_audio_test(dataset)

#print(len(dataset_test['linear']['inspec']))
#print(len(dataset_test['reverb']['inspec']))
#print(len(dataset_test['mp3']['inspec']))

#inaudio, outaudio, label = load_full_data(dataset,'train',0)
...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    enhanced = featureloss_train(reshaped_waveform,reshaped_waveform_1,loss_weights,loss_layers=SE_LOSS_LAYERS,n_layers=LOSS_LAYERS, norm_type=LOSS_NORM, base_channels=LOSS_BASE_CHANNELS,blk_channels=LOSS_BLK_CHANNELS) 
...
def scores_map(noise='mp3'):
    ...
    for id in tqdm(range(0, len(dataset_test["all"]["inname"]))):
        ...
        spec_in,spec_out,wav_in,wav_out=load_full_data_test(dataset_test,'all',id)
        a,_= sess.run([distance,enhanced1],feed_dict={input1_spec:spec_in, clean1_spec:spec_out, input1_wav:wav_in, clean1_wav:wav_out,loss_weights:loss_w})
        output[id]=a[0]
...
with tf.Session() as sess:
    ...
    for epoch in range(epoches):
        ...
        for j in tqdm(range(batches)):
            ...
            spec_in,spec_out,wav_in,wav_out,labels=load_full_data(dataset_train,'train',j)
            ...
            for i in range(labels.shape[0]):
                if float(labels[i])==0:
                    ...
                    y[i]+=[0,1]
            loss_w=np.ones([SE_LOSS_LAYERS])
            _,dist,loss_train= sess.run([opt_task,distance,loss_1],feed_dict={input1_spec:spec_in, clean1_spec:spec_out, input1_wav:wav_in, clean1_wav:wav_out,loss_weights:loss_w,label_task:y})
        ...
        if epoch%10==0:
            ...
            n_batches = batches // 1
            for j in tqdm(range(batches)):
                spec_in,spec_out,wav_in,wav_out,labels=load_full_data(dataset_test,'test',j)
                y=np.zeros((labels.shape[0],2))
                for i in range(labels.shape[0]):
                    if float(labels[i])==0:
                        y[i]+=[1,0]
                        ...
                        y[i]+=[0,1]
                loss_w=np.ones([SE_LOSS_LAYERS])
                dist,loss_train_1= sess.run([distance,loss_1],feed_dict={input1_spec:spec_in, clean1_spec:spec_out, input1_wav:wav_in, clean1_wav:wav_out,loss_weights:loss_w,label_task:y})
                loss_epoch_test.append(loss_train_1)
