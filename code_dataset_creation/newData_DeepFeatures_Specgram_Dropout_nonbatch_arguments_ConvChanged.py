...
def argument_parser():
    ...
    parser.add_argument('--kernel_size', help='kernel size for convolution', default=3, type=int)
...
def load_full_data(dataset,sets,id_value):
    ...
    '''
    return [inputData_spec,outputData_spec,label]
...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    enhanced = featureloss_train(input1_spec,clean1_spec,loss_weights,loss_layers=SE_LOSS_LAYERS,n_layers=LOSS_LAYERS, norm_type=LOSS_NORM, base_channels=LOSS_BASE_CHANNELS,blk_channels=LOSS_BLK_CHANNELS) 
...
def scores_map(noise='mp3'):
    ...
    for id in tqdm(range(0, len(dataset_test["all"]["inname"]))):
        ...
        spec_in,spec_out=load_full_data_test(dataset_test,'all',id)
        a,_= sess.run([distance,enhanced1],feed_dict={input1_spec:spec_in, clean1_spec:spec_out, loss_weights:loss_w})
        output[id]=a[0]
...
with tf.Session() as sess:
    ...
    for epoch in range(epoches):
        ...
        for j in tqdm(range(batches)):
            ...
            spec_in,spec_out,labels=load_full_data(dataset_train,'train',j)
        ...
        if epoch%10==0:
            ...
            n_batches = batches // 1
            for j in tqdm(range(batches)):
                spec_in,spec_out,labels=load_full_data(dataset_test,'test',j)
                y=np.zeros((labels.shape[0],2))
                for i in range(labels.shape[0]):
                    if float(labels[i])==0:
                        y[i]+=[1,0]
                        ...
                        y[i]+=[0,1]
                loss_ones=np.ones([SE_LOSS_LAYERS])
                dist,loss_train_1= sess.run([distance,loss_1],feed_dict={input1_spec:spec_in, clean1_spec:spec_out, loss_weights:loss_ones,label_task:y})
