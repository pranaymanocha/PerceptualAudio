...
def featureloss(target, current, keep_prob ,loss_layers, n_layers=14, norm_type="SBN", base_channels=32, blk_channels=5,ksz=3):
    ...
    for id in range(loss_layers):
        ...
        a1=tf.reshape(a,[n,1,channels[id],-1])
...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    enhanced = featureloss(input1_wav,clean1_wav,keep_prob,loss_layers=SE_LOSS_LAYERS,n_layers=LOSS_LAYERS, norm_type=LOSS_NORM, base_channels=LOSS_BASE_CHANNELS,blk_channels=LOSS_BLK_CHANNELS,ksz=FILTER_SIZE) 
